	...
	if(isset($_POST['registerUsername'])){
		...
		if(!empty($registerFullName) && !empty($registerUsername) && !empty($registerPassword1) && !empty($registerPassword2)){
			...
			if($usernameCheckingStatement->rowCount() > 0){
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>This Username is Already used. Please select a different username.</div>';
				exit();
			} else {
				...
				if($registerPassword1 !== $registerPassword2){
					...
					echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>User Added Successfully.</div>';
					exit();
				}
			}
		} else {
